...
function timelineBar(report) {
    ...
    const isOverdue = now > deadline && report.status !== 'Submitted';
    const barColor = report.status === 'Submitted' ? '#16a34a' : isOverdue ? '#dc2626' : pct > 75 ? '#d97706' : '#2563eb';

    return `
        <div style="background:#f3f4f6;border-radius:4px;height:6px;width:100%;margin-top:0.4rem;overflow:hidden;">
            <div style="background:${barColor};height:100%;width:${report.status === 'Submitted' ? 100 : pct}%;transition:width 0.3s;"></div>
        </div>
...
function renderSAEPage(reports, overdue, role) {
    ...
    const rows = reports.length === 0
        ? '<tr><td colspan="7" style="padding:2rem;text-align:center;color:#6b7280;">No SAE reports yet.</td></tr>'
        : reports.map(r => `
            <tr style="cursor:pointer;" class="sae-row" data-id="${r.id}">
                ...
                <td style="padding:0.7rem 0.75rem;border-bottom:1px solid #f3f4f6;text-align:right;">
                    ...
                    <button class="btn-submit-sae" data-id="${r.id}"
                        style="background:#059669;color:#fff;border:none;border-radius:6px;padding:0.3rem 0.75rem;cursor:pointer;font-size:0.8rem;">
                        Mark Submitted
                    </button>` : ''}
                </td>
            </tr>
